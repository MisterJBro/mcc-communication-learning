...
class Speaker(nn.Module):
    ...
    def next_action(self, x, state):
        ...
        x = h_n[-1]
        action_dist = Categorical(probs=self.action(x))
        message = F.gumbel_softmax(self.message(x)/0.3, self.tau, hard=True)
    ...
    def forward(self, x):
        ...
        action_dists = Categorical(probs=self.action(x))
        messages = F.gumbel_softmax(self.message(x)/0.3, self.tau, hard=True)
        values = self.value(x)
        return action_dists, messages, values
    ...
    def message_only(self, x):
        x = self.tail(x)
        return F.gumbel_softmax(self.message(x)/0.3, self.tau, hard=True)
